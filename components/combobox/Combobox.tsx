	...
	return (
		<Popover open = { isOpen ? true : open } onOpenChange = { setOpen }>
			<PopoverTrigger
				...
				disabled  = { disabled }
				className = { cn(
					'w-full inline-flex items-center justify-between min-h-10 rounded-xl border border-border bg-background px-4 py-2 text-sm text-foreground hover:bg-muted/40 transition-colors disabled:cursor-not-allowed disabled:opacity-50',
					className
				) }
			>
			...
			>
				...
				>
					{ isLoading ? (
						...
						<List<any>
							listRef      = { listRef }
							style        = { { height : Math.min( filteredItems.length * ITEM_HEIGHT, MAX_HEIGHT ), width : '100%' } }
